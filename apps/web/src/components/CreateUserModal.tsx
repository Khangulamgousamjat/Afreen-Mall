...
export const CreateUserModal: React.FC<CreateUserModalProps> = ({ onClose, onCreated }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onCreated(res.data.user, res.data.oneTimeTemporaryPassword || 'Pass@123');
    } catch (err: any) {
      ...
      <div className="modal-content" style={{ maxWidth: '560px', maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Staff ID will be auto-assigned. Default password: Pass@123</div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div style={{ backgroundColor: 'rgba(16,185,129,0.05)', border: '1px solid rgba(16,185,129,0.2)', padding: '12px', fontSize: '12px', color: 'var(--text-muted)' }}>
            ...
            Staff ID auto-assigned from 300000+. Default password <strong>Pass@123</strong> — user must change on first login.
          </div>
