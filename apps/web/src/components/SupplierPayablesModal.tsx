...
export const SupplierPayablesModal: React.FC<SupplierPayablesModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await api.post('/suppliers/payments', payload);
      setReceipt(res.data);
      setTimeout(() => {
        onSuccess();
        onClose();
      ...
      <div className="modal-content" style={{ maxWidth: '540px', border: '2px solid var(--accent-lime)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <h3 style={{ fontSize: '18px', fontWeight: 'bold', textTransform: 'uppercase' }}>
              Accounts Payable Vendor Payment Settlement
            </h3>
          </div>
        ...
        {receipt && (
          <div style={{ padding: '12px', backgroundColor: 'rgba(16,185,129,0.15)', color: 'var(--status-green)', border: '1px solid var(--status-green)', fontSize: '13px', marginBottom: '14px', textAlign: 'center', fontWeight: 'bold' }}>
            ✓ Receipt {receipt.receiptNo} generated! ₹{parseFloat(amountRupees).toLocaleString('en-IN')} paid to {supplierName}.
          </div>
        )}

        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <div>
            <label style={{ fontSize: '11px', fontWeight: 'bold', color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>Select Vendor / Supplier *</label>
            <input type="text" className="input-field" value={supplierName} onChange={(e) => setSupplierName(e.target.value)} required />
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            <div>
              <label style={{ fontSize: '11px', fontWeight: 'bold', color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>Payment Settlement Amount (₹) *</label>
              <input
                type="number"
                step="0.01"
                ...
                placeholder="e.g. 54000.00"
                required
              />
            </div>

            <div>
              <label style={{ fontSize: '11px', fontWeight: 'bold', color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>Payment Channel</label>
              <select className="input-field" value={paymentMode} onChange={(e) => setPaymentMode(e.target.value)}>
                ...
                <option value="CHEQUE">Bank Account Cheque</option>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            <div>
              <label style={{ fontSize: '11px', fontWeight: 'bold', color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>Bank UTR / Cheque Ref</label>
              <input type="text" className="input-field tabular-nums" value={referenceNo} onChange={(e) => setReferenceNo(e.target.value)} />
            </div>

            <div>
              <label style={{ fontSize: '11px', fontWeight: 'bold', color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>Invoice / PO Notes</label>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '10px' }}>
            ...
            <button type="submit" className="btn btn-primary" disabled={loading || !!receipt} style={{ padding: '8px 20px' }}>
              {loading ? 'Settling...' : 'Settle Vendor Payable & Issue Receipt'}
            </button>
          </div>
        </form>
      </div>
