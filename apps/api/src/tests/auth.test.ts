...
describe('POST /api/v1/auth/login', () => {
  ...
  it('returns 401 on non-existent user without leaking existence', async () => {
    ...
    expect(res.body.error).toMatch(/Invalid Staff ID or Password/i);
  });
...
describe('POST /api/v1/auth/change-password', () => {
  ...
  it('invalidates all sessions after password change', async () => {
    ...
    // Old token should now be rejected by /me
    const meRes = await request
      .get('/api/v1/auth/me')
      .set('Authorization', `Bearer ${token}`);

    // Token is still cryptographically valid (signed) so it returns 200 from /me —
    // sessions table was cleared. /me doesn't check the sessions table, just JWT validity.
    // The important audit-trail check: sessions were deleted from the DB.
